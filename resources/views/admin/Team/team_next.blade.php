    ...
    <section class="content">
                ...
                <div class="col-lg-12 col-md-12 col-sm-12">
                    <div class="more-sports">
                        <div class="more-sports-heading">
                            <h3 class="text-center">{{ $team_id->team }} Matches Page </h3>
                            ...
                            <div class="demo" id="matches_demo" style="display: none!important;">
                                <form action="{{ url('/team_next_set') }}" method="post" class="form-inline">
                        ...
                        <table class="table table-bordered" style="border: 1px solid black">
                            ...
                            <tbody>
                            @if(isset($team_id))
                                @foreach($team_id->Team_next as $match)
                                    <tr>
                                        <td>{{ $i++ }}</td>
                                        <td>{{ $match->date }}</td>
                                        <td>{{ $match->team1 }}</td>
                                        <td>{{ $match->team2 }}</td>
                                        <td>
                                            <button type="button" class="btn btn-success matches_update" data-toggle="modal" data-target="#matches_update" data-id="{{ $match->id }}" data-teamid="{{ $match->team_id }}"  data-date="{{ $match->date }}" data-team1="{{ $match->team1 }}"  data-team2 ="{{ $match->team2 }}"><span class="glyphicon glyphicon-edit"></span></button>
                                            <a href="{{ url('/team_next_del', $match->id) }}"><button type="button" class="btn btn-danger "><span class="glyphicon glyphicon-trash"></span></button></a>
                                        </td>
                                    </tr>
                                @endforeach
                            @endif
                        ...
                        <div class="modal-dialog modal-lg" >
                            <div class="modal-content col-sm-12">
                                ...
                                <div class="modal-body">
                                    <form action="{{ url('/team_next_edit') }}" method="post" class="form-inline">
                                        <input type="hidden" name="_token" value="{{ csrf_token() }}">
                                        <input type="hidden" name="matches_id_hid" id="matches_id_hid" value="" />
                                        <input type="hidden" name="team_id" id="matches_teamid" value="" />
                                        <div class="form-group">
                                            <label for="Date">Date</label>
        ...
        <script>
            ...
            $('.matches_update').on('click', function () {
                ...
                var $matches_teamid =   $(this).data('teamid') ;
                ...
                $('#matches_teamid').val($matches_teamid);
